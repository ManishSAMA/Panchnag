...
class TestDainikaRules:
    ...
    def test_names_unique_except_intentional_duplicates(self):
        ...
        counts = Counter(r["name"] for r in DAINIKA_RULES)
        actual_dupes = {name: n for name, n in counts.items() if n > 1}
        expected_dupes = {"Raj Yog": 3, "Kumar Yog (Tyajya)": 4}
        assert actual_dupes == expected_dupes, \
            f"Unexpected duplicate rule names: {actual_dupes}"
