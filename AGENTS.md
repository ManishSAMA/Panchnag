DO not make changes until you have 95% confidence in what you need to build , as me followup until you reach the confidence
...
**Project-level hooks:** Projects should add a PostToolUse hook in `.Codex/settings.json` to run typecheck after Write/Edit on .ts/.tsx files. Use `/setup` to generate this automatically, or see the global `settings.json` prettier/eslint hook as a template.
